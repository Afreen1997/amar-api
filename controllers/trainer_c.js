...
let trainer_model = require('../models/trainer_m')
...
select_all: async (req, res)=>{// Get all users
    try {
        let trainers = await trainer_model.find().exec()
        res.send(trainers)
    } catch(err) {
...
select1_by_id: async (req, res)=>{// Get a selected user
    try {
        let trainer = await trainer_model.findById(req.params.id).exec()
        res.send(trainer)
    } catch(err) {
        res.status(500).send(err)
    }
...
register: async(req, res)=>{// Save an user Record
    try {
        ...
        let trainer = new trainer_model(req.body)
        let result = await trainer.save()
        res.send(result)
    } catch(err) {
        res.status(500).send(err)
    }
...
authenticate: async(req, res)=>{// Check valid user or not
    try {
        console.log(req.body)
        let trainers= await trainer_model.find({ email_id: req.body.email_id, pass_word: req.body.pass_word }).exec()
        //console.log(users.length)
        //console.log(users)
        if(trainers.length == 1) {// Found User record for given email_id & pass_word
...
delete1: async(req,res)=>{// Delete an user Record
    try {
        let result = await trainer_model.deleteOne({_id: req.params.id}).exec()
        res.send(result)
    } catch(err) {
        res.status(500).send(err)
    }
...
update1: async(req, res)=>{// Update an user Record
    try {
        ...
        let result = await trainer_model.findOneAndUpdate(filter, update, {new: true});
        res.send(result)
    } catch(err) {
        res.status(500).send(err)
    }
