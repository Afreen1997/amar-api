...
let commission_model = require('../models/commission_m')
...
select_all: async (req, res)=>{// Get all users
    try {
        let commissions = await commission_model.find().exec()
...
select1_by_id: async (req, res)=>{// Get a selected user
    try {
        let commission = await commission_model.findById(req.params.id).exec()
        res.send(commission)
    } catch(err) {
        res.status(500).send(err)
    }
...
register: async(req, res)=>{// Save an user Record
    try {
        ...
        let commission = new commission_model(req.body)
...
authenticate: async(req, res)=>{// Check valid user or not
    try {
        console.log(req.body)
        let commissions= await commission_model.find({ email_id: req.body.email_id, pass_word: req.body.pass_word }).exec()
        //console.log(users.length)
        //console.log(users)
        if(commissions.length == 1) {// Found User record for given email_id & pass_word
...
delete1: async(req,res)=>{// Delete an user Record
    try {
        let result = await commission_model.deleteOne({_id: req.params.id}).exec()
        res.send(result)
    } catch(err) {
        res.status(500).send(err)
    }
...
update1: async(req, res)=>{// Update an user Record
    try {
        ...
        let result = await commission_model.findOneAndUpdate(filter, update, {new: true});
        res.send(result)
    } catch(err) {
        res.status(500).send(err)
    }
