...
let currenttraining_model = require('../models/currenttraining_m')
...
select_all: async (req, res)=>{// Get all users
    try {
        let currenttrainings = await currenttraining_model.find().exec()
        res.send(currenttrainings)
    } catch(err) {
...
records_in_table_form: async (req, res)=>{// Get all users & List in HTML Table
    try {
        ...
        let serial_no = 1
        currenttrainings.forEach((record)=>{
...
select1_by_id: async (req, res)=>{// Get a selected user
    try {
        let currenttraining = await currenttraining_model.findById(req.params.id).exec()
...
register: async(req, res)=>{// Save an user Record
    try {
        ...
        let currenttraining = new currenttraining_model(req.body)
        let result = await currenttraining.save()
        res.send(result)
    } catch(err) {
        res.status(500).send(err)
    }
...
authenticate: async(req, res)=>{// Check valid user or not
    try {
        console.log(req.body)
        let currenttrainings= await currenttraining_model.find({ email_id: req.body.email_id, pass_word: req.body.pass_word }).exec()
        //console.log(users.length)
        //console.log(users)
        if(currenttrainings.length == 1) {// Found User record for given email_id & pass_word
...
delete1: async(req,res)=>{// Delete an user Record
    try {
        let result = await currenttraining_model.deleteOne({_id: req.params.id}).exec()
        res.send(result)
    } catch(err) {
        res.status(500).send(err)
    }
...
update1: async(req, res)=>{// Update an user Record
    try {
        ...
        let result = await currenttraining_model.findOneAndUpdate(filter, update, {new: true});
        res.send(result)
    } catch(err) {
        res.status(500).send(err)
    }
